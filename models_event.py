...
def gaussian_encode(x: np.array, k,  n, m, mu, sigma2, max_spike_time=50):
    ...
    x = tf.reshape(x, [x_shape2[0]*x_shape2[1], n], name='x_reshape') # shape=[batch_size * k, n]
    ...
    out_spikes += tf.floor(max_spike_time * (1 - y)+0.5, name='spikes_round') * tf.cast(y>0.5/max_spike_time, tf.float32) # [batch_size * k, n, m]
 
    out_spikes = tf.reshape(out_spikes, [x_shape[0], k, n, m], name='out_spikes_reshape') # shape: [batch_size * k, n, m] -> [batch_size, k, n, m]
