...
import torchvision
import numpy as np
sys.path.append("/home/jliu/codes/spikingjelly")
...
def main(args):
    ...
    * :ref:`中文API <lif_fc_mnist.main-cn>`
    ...
    dataloader_type = 'torch' # torch, numpy
    ...
    elif dataloader_type == 'numpy':
        try:
            train_samples = np.load('train_samples.npy')
            train_labels = np.load('train_labels.npy')
        except Exception:
            train_samples, train_labels = load_mnist('/home/jliu/codes/MNIST/raw', 'train')
            np.save('train_samples.npy', train_samples)
            np.save('train_labels.npy', train_labels)
        train_data_loader = NpDataloader(train_samples, train_labels, batch_size, None, None)

        try:
            test_samples = np.load('test_samples.npy')
            test_labels = np.load('test_labels.npy')
        except Exception:
            test_samples, test_labels = load_mnist('/home/jliu/codes/MNIST/raw', 'test')
            np.save('test_samples.npy', test_samples)
            np.save('test_labels.npy', test_labels)
        test_data_loader = NpDataloader(test_samples, test_labels, batch_size, None, None)
